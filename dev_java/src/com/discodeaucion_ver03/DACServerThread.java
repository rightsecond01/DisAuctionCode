...
public class DACServerThread extends Thread {
	...
	private int countFileNum(String room_game) {
		int fileNum = 0;
				...
				case Dprotocol.ROOM_IN: {
					...
					if(isRoomFull) {
						...
						if(isInRoom) {//isInRoom이 true => 이미 roomin한 상태.
							...
							ImagesTransmission it = new ImagesTransmission(inRoomUserList);
							it.start();
							if(room_type.equals("AuctionRoom")) {
								PreImageTransmission pit = new PreImageTransmission(room_game);
								pit.start();								
							}
