...
public class ClientCtrl extends JFrame implements Runnable {
	...
	public void setPreFriend(String friendInfo) {
		...
		for(int i=0; i<fInfoArray.size();i++) {
			String fri_id = fInfoArray.get(i).getAsJsonObject().get("fri_id").getAsString();
			String fri_name = fInfoArray.get(i).getAsJsonObject().get("fri_name").getAsString();
			try {
				main_oos.writeObject(Dprotocol.ISONLINE
	...
	public void initDisplay() {
		...
		tp.addTab("메인화면", mv);
		con.add(tp, null);
		this.getContentPane().setBackground(new Color(84,84,84));
